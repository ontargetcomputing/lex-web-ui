<template>
  ...
  >
    ...
    <!-- v-bind:src="require(`@/assets/${toolbarLogo}`)" -->
    <!-- v-if="toolbarLogo" -->
    <img
    ...
    <!-- <v-menu v-if="showToolbarMenu" offset-y>
      ...
      <v-list>
        ...
        <v-list-tile v-if="isSaveHistory">
          <v-list-tile-title
            v-on:click="requestResetHistory"
        ...
        <v-list-tile v-if="canLiveChat">
          <v-list-tile-title
            v-on:click="requestLiveChat"
            aria-label="request live chat"
          >
            <v-icon>
              {{ items[5].icon }}
            </v-icon>
    ...
    <span v-if="isLocaleSelectable" class="localeInfo">{{
      currentLocale
    }}</span>
    <v-btn
    ...
    <!-- <v-btn
      v-if="this.$store.state.isRunningEmbedded"
      v-on:click.stop="toggleMinimize"
